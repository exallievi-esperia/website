...
import { EventType } from '@/pages/eventi'
...
export interface EventiCardProps {
  id: number
  name: string
  description: string
  date: Date
    ...
    >
      {image && (
        <Image
          ...
          className={`object-cover ${className} rounded-lg`}
        />
      )}
      ...
      <div className="z-10 flex flex-col ">
        ...
        <p className="block text-gray-500 text-base dark:text-gray-100 mb-4">{description}</p>
        <span className="flex items-center text-gray-500 text-sm mt-auto">
          ...
          <p className="dark:text-gray-300">{date.day + ' ' + date.month + ' ' + date.year}</p>
        </span>
      </div>
